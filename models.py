...
class User(Base):
    ...
    vk_id: Mapped[str] = mapped_column(String(length=15), unique=True)
...
class Candidate(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    vk_id: Mapped[str] = mapped_column(String(length=15), unique=True)
    first_name: Mapped[str] = mapped_column(String(length=15), nullable=True)
    last_name: Mapped[str] = mapped_column(String(length=15), nullable=True)
...
class Photo(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    candidate_id: Mapped[str] = mapped_column(String(length=15), ForeignKey('Candidates.vk_id'), nullable=False)
...
class Reaction(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    user_id: Mapped[str] = mapped_column(String(length=15), ForeignKey('Users.vk_id'), nullable=False)
    candidate_id: Mapped[str] = mapped_column(String(length=15), ForeignKey('Candidates.vk_id'), nullable=False)
